...
export function SkillConfigDialog({
  ...
  const heroHeader = (
    <div
      className="px-6 pt-6 pb-5 flex flex-col gap-4 shrink-0"
    ...
    >
      ...
      {(skill.website || skill.docs) && (
        <div className="flex items-center gap-4">
          ...
          {skill.docs && (
            <a href={skill.docs} target="_blank" rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 text-xs font-medium transition-opacity hover:opacity-70"
              style={{ color: skill.color }}>
              <BookOpen className="size-3.5" />Документация
            </a>
          )}
        </div>
      )}
  ...
  const configForm = skill.fields.length > 0 ? (
    <div className="px-6 py-5 flex flex-col gap-4 border-t border-border/40 shrink-0">
      <div className="flex items-center justify-between">
        ...
        {hasInstructions && (
          ...
          >
            ...
            <ChevronRight className={cn("size-3.5 transition-transform", showInstructions && "rotate-180")} />
          </button>
        )}
      </div>
    ...
    <div className="px-6 py-5 border-t border-border/40 shrink-0">
  ...
  const instructionsPanel = skill.instructions ? (
    <div className="flex flex-col h-full" style={!isMobile ? { width: "380px" } : undefined}>
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      {isMobile ? (
        ...
        <DialogContent className="p-0 gap-0 max-w-[95vw] w-full overflow-hidden flex flex-col max-h-[90vh]">
        ...
        // ── Desktop: side-by-side, instructions animate to the right ───────
        <DialogContent
